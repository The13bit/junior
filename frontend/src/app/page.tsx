...
const Page = () => {
  const { claim, balanceOf, totalSupply, loading, setWallet,Transfer } = useKalpApi();
  const [walletAddress, setWalletAddress] = useState("");
  const [balance, setBalance] = useState(0);
  const [totalAirdrop, setTotalAirdrop] = useState(0);
  const [reciversAddr,setReciverAddr]=useState("")
  const [reciversAddrAmount,setReciverAddrAmount]=useState(0)
  const [tmpadr,settempaddr]=useState("");
  ...
  const handleBalanceOf = async () => {
    try {
      const data = await balanceOf(tmpadr);
    ...
    <div className="flex flex-col justify-center items-center">
      ...
      <div className="lg:flex gap-12">
        <div className="flex flex-col border-2 p-8 mt-8 rounded-lg w-fit gap-3">
          ...
          <p className="text-6xl text-blue-500 font-bold w-56">
            {totalAirdrop}
          </p>
        </div>
        ...
        <div className="flex flex-col border-2 p-8 mt-8 rounded-lg w-fit gap-3">
          ...
          <p className="text-2xl font-bold w-56">
            Balance: <span className="text-blue-500 text-4xl"> {balance}</span>
          </p>
        </div>
        ...
        <div className="flex flex-col border-2 p-8 mt-8 rounded-lg w-fit gap-3">
          ...
          <input
            ...
            type="text"
            className="border p-2 rounded-lg w-56 text-black"
            onChange={(e) => setReciverAddr(e.target.value)}
          />
          Amount:
          <input
            placeholder="Enter your wallet address"
            type="text"
            className="border p-2 rounded-lg w-56 text-black"
            onChange={(e) => setReciverAddrAmount(parseInt(e.target.value))}
          />
          <button
            className="border-2 p-2 rounded-lg bg-blue-500 hover:bg-blue-400 text-white"
            onClick={()=>{
              Transfer(reciversAddr,reciversAddrAmount)
            }}
          >
            Send
          </button>
